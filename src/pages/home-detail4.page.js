...
export default class HomeDetail4 extends Component {
  ...
  state = {
    ...
    storedata = async () =>{
        ...
        let newItems = [...this.state.item4, { id: this.makeid(10), data: this.state.text }];

	
		await AsyncStorage.setItem("mylist4", JSON.stringify(newItems))
  ...
  render(){
    if(this.state.item4.length > 0){
      ...
      return (
		    <TouchableOpacity key={item4.id} onPress={
			  () => {
                  this.deleteItem(item4.id);
