...
export default class HomeDetail6 extends Component {
  ...
  state = {
    ...
    storedata = async () =>{
        ...
        let newItems = [...this.state.item6, { id: this.makeid(10), data: this.state.text }];

	
		await AsyncStorage.setItem("mylist6", JSON.stringify(newItems))
  ...
  render(){
    if(this.state.item6.length > 0){
      ...
      return (
		    <TouchableOpacity key={item6.id} onPress={
			  () => {
                  this.deleteItem(item6.id);
