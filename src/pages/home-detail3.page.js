...
export default class HomeDetail3 extends Component {
  ...
  state = {
    ...
    storedata = async () =>{
        ...
        let newItems = [...this.state.item3, { id: this.makeid(10), data: this.state.text }];

	
		await AsyncStorage.setItem("mylist3", JSON.stringify(newItems))
  ...
  render(){
    if(this.state.item3.length > 0){
      ...
      return (
		    <TouchableOpacity key={item3.id} onPress={
			  () => {
                  this.deleteItem(item3.id);
