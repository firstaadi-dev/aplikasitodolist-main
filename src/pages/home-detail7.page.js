...
export default class HomeDetail1 extends Component {
  ...
  state = {
    ...
    storedata = async () =>{
        ...
        let newItems = [...this.state.item7, { id: this.makeid(10), data: this.state.text }];

	
		await AsyncStorage.setItem("mylist7", JSON.stringify(newItems))
  ...
  render(){
    if(this.state.item7.length > 0){
      ...
      return (
		    <TouchableOpacity key={item7.id} onPress={
			  () => {
                  this.deleteItem(item7.id);
